...
# ):
#     try:
#         # Fetch promotional offers from the collection
#         promotional_offers_collection = get_collection(PROMOTIONAL_OFFERS_COLLECTION_NAME)
#         promotional_offers = await promotional_offers_collection.find({}).to_list(length=None)
...
#         # Process branchwise and order type details
#         branchwise_info = {}
#         for key, value in cleaned_item.items():
#             if key.startswith("Price_") or key.startswith("EnablePrice_") or key.startswith("systemStock_") or key.startswith("physicalStock_"):
...
#                 branchwise_info.setdefault(branch, {})[attribute] = value

#         # Check for matching promotional offers and include them directly
#         for branch, branch_data in branchwise_info.items():
#             branch_price = branch_data.get("Price", 0)
#             matched_offer = None
#             for offer in promotional_offers:
#                 if branch in offer.get("locations", []):
#                     if (
#                         item_name in offer.get("itemName", []) or
#                         variance_name in offer.get("varianceName1", []) or
#                         variance_name in offer.get("varianceName2", []) or
#                         cleaned_item.get("category") in offer.get("category", []) or
#                         cleaned_item.get("subcategory") in offer.get("subcategory", [])
#                     ):
#                         try:
#                             discount_value = float(offer.get("discountValue", 0) or 0)  # Default to 0 if empty
#                         except ValueError:
#                             discount_value = 0  # Handle invalid values

#                         final_price = round(branch_price - (branch_price * discount_value / 100), 2)
...
#                             "selectionType": offer.get("selectionType", ""),
#                             "buy1Get1": {
#                                 "buyVariance": offer.get("varianceName1", []),
#                                 "getVariance": offer.get("varianceName2", [])
#                             },
#                             "buy": offer.get("buy", ""),
#                             "get": offer.get("get", "")
#                         }
#                         break  # Exit after the first matching offer

#             # Add matched offer details directly under 'physicalStock'
#             if matched_offer:
#                 branch_data[""] = matched_offer

#         # Update variance with branchwise and offers info
...
#             for offer in promotional_offers:
#                 if branch in offer.get("locations", []):
#                     if offer.get("freeoffer", "false") == "true":
#                         matched_offers.append({
#                             "offerType": offer.get("offerType"),
#                             "offerName": offer.get("offerName"),
#                             "configuration": offer.get("configuration"),
#                             "selectionType1": offer.get("selectionType1"),
#                             "selectionType2": offer.get("selectionType2"),
#                             "varianceName1": offer.get("varianceName1", []),
#                             "varianceName2": offer.get("varianceName2", []),
#                             "category1": offer.get("category1", []),
#                             "category2": offer.get("category2", []),
#                             "subcategory1": offer.get("subcategory1", []),
#                             "subcategory2": offer.get("subcategory2", []),
#                             "buy": offer.get("buy", 0),
#                             "get": offer.get("get", 0)
#                         })
...
#                             "configuration": offer.get("configuration"),
#                             "selectionType": offer.get("selectionType"),
#                             "varianceName": offer.get("varianceName", []),
...
#             # Add matched offers to branch data
#             if matched_offers:
#                 branch_data["offers"] = matched_offers

#         # Update variance with branchwise info
#         result[item_name]["variance"].setdefault(variance_name, {}).update({
#             **variance_info,
#             "branchwise": branchwise_info
#         })
...
#         # Filter offers that match the current item and branch
#         for branch, branch_data in branchwise_info.items():
#             branch_price = branch_data.get("Price", 0)
...
#         # Ensure freeoffer and discountOffer default to "false"
#         for branch in branchwise_info.keys():
#             branchwise_info[branch]["freeoffer"] = "false"
#             branchwise_info[branch]["discountOffer"] = "false"
#             branchwise_info[branch]["finalPrice"] = branchwise_info[branch].get("Price", 0)  # Default finalPrice to Price
...
#                     subcategory in offer.get("subcategory", [])
#                 ):
#                     # Add promotional offer fields
#                     branch_data.update({
#                         "appTypes": offer.get("appTypes", []),
#                         "offerName": offer.get("offerName"),
#                         "locations": offer.get("locations", []),
#                         "startDate": offer.get("startDate"),
#                         "endDate": offer.get("endDate"),
#                         "fromTime": offer.get("fromTime"),
#                         "toTime": offer.get("toTime"),
...
#                         "orderValue": offer.get("orderValue"),
#                         "orderDiscountValue": offer.get("orderDiscountValue"),
#                         "customers": offer.get("customers", []),
#                         "image": offer.get("image"),
#                         "selectionType1": offer.get("selectionType1"),
#                         "selectionType2": offer.get("selectionType2"),
#                         "itemName1": offer.get("itemName1", []),
#                         "itemName2": offer.get("itemName2", []),
#                         "varianceName1": offer.get("varianceName1", []),
#                         "varianceName2": offer.get("varianceName2", []),
#                         "category1": offer.get("category1", []),
#                         "category2": offer.get("category2", []),
...
#                         "freeoffer": offer.get("freeoffer", "false"),
#                         "discountOffer": offer.get("discountOffer", "false")
#                     })
...
#                         final_price = int(round(branch_price - (branch_price * discount_value / 100)))
#                         branch_data["finalPrice"] = str(final_price)
...
#                        # Handle discount offers: update finalPrice with branch-specific key
#             if offer.get("discountOffer", "false") == "true":
#                 try:
#                     discount_value = float(offer.get("discountValue", 0) or 0)
#                 except ValueError:
...
#         del items, promotional_offers, cleaned_item, branchwise_info, variance_info, offer, branch_data
...
#             # Process branchwise and order type details using branch-suffixed keys
#             branchwise_info = {}
#             for key, value in cleaned_item.items():
#                 if key.startswith(("Price_", "EnablePrice_", "systemStock_", "physicalStock_")):
#                     parts = key.split("_")
...
#             # Ensure freeoffer and discountOffer default to "false" for each branch,
#             # and set a branch-specific finalPrice
#             for branch in branchwise_info.keys():
#                 branchwise_info[branch][f"freeoffer_{branch}"] = "false"
#                 branchwise_info[branch][f"discountOffer_{branch}"] = "false"
#                 branchwise_info[branch][f"finalPrice_{branch}"] = branchwise_info[branch].get(f"Price_{branch}", 0)

#             # Filter and update offers that match the current item and branch
#             for branch, branch_data in branchwise_info.items():
#                 branch_price = branch_data.get(f"Price_{branch}", 0)
...
#                         subcategory in offer.get("subcategory", [])
#                     ):
#                         # Add promotional offer fields with branch-suffixed keys
#                         branch_data.update({
...
async def get_branchwise_promotional_items(
    ...
    try:
        for item in items:
            ...
            # Clean the item data (e.g., replace NaN floats with None)
            cleaned_item = {
                k: (None if isinstance(v, float) and np.isnan(v) else v)
                for k, v in item.items()
            }
