...
from .utils import get_purchaseitem_collection  # Adjust import as per your project structure
import csv
import logging
...
async def create_purchaseitem(purchaseitem_data: PurchaseItemPost):
    try:
        ...
        new_purchaseitem_data['createdDate'] = current_date_and_time['datetime']  # Add created date
...
@router.get("/export_csv", response_model=dict, operation_id="export_purchase_items_to_csv")
async def export_all_purchase_items_to_csv():
    try:
        ...
        headers = [
            'itemName', 'purchasecategoryName', 'purchasesubcategoryName', 'itemgroupName', 
            'uom', 'stockQuantity', 'supplier', 'purchasePrice', 'purchasetaxName', 
            'reorderLevel', 'itemType', 'hsnCode', 'shelfLife', 'vendorTag', 
            'locationName', 'barcode', 'description', 'createdDate', 
...
async def get_all_items(skip: int = Query(0, ge=0), limit: int = Query(50, le=5000)):
    try:
        cursor = get_purchaseitem_collection().find().skip(skip).limit(limit)
...
async def search_items(itemName: Optional[str] = None):
    try:
        ...
        if itemName:
            query['itemName'] = {"$regex": itemName, "$options": "i"}  # Case-insensitive search
...
async def get_purchaseitem_by_id(purchaseitem_id: str):
    purchaseitem = get_purchaseitem_collection().find_one({"_id": ObjectId(purchaseitem_id)})
    if purchaseitem:
...
async def update_purchaseitem(purchaseitem_id: str, purchaseitem_data: PurchaseItemPost):
    try:
        updated_purchaseitem = purchaseitem_data.dict(exclude_unset=True)  # exclude_unset=True prevents sending None values to MongoDB
        current_date_and_time = get_current_date_and_time()
        updated_purchaseitem['lastUpdatedDate'] = current_date_and_time['datetime']

        result = get_purchaseitem_collection().replace_one({"_id": ObjectId(purchaseitem_id)}, updated_purchaseitem)
...
async def patch_purchaseitem(purchaseitem_id: str, purchaseitem_patch: PurchaseItemPost):
    try:
        existing_purchaseitem = get_purchaseitem_collection().find_one({"_id": ObjectId(purchaseitem_id)})
        ...
        updated_purchaseitem = get_purchaseitem_collection().find_one({"_id": ObjectId(purchaseitem_id)})
...
async def delete_purchaseitem(purchaseitem_id: str):
    result = get_purchaseitem_collection().delete_one({"_id": ObjectId(purchaseitem_id)})
...
async def import_purchase_items_from_csv(file: UploadFile = File(...)):
    try:
        ...
        last_item = get_purchaseitem_collection().find_one(
            sort=[("randomId", -1)]
        )
        last_random_id = 0
        ...
        if last_item and last_item.get("randomId"):
            try:
                last_random_id = int(last_item['randomId'][2:])  # Extract the numeric part of the last randomId
            except ValueError:
                last_random_id = 0
        ...
        for idx, row in enumerate(csv_reader):
            ...
            purchase_item_data = {
                ...
                'uom': row.get('uom'),
                'stockQuantity': float(row['stockQuantity']) if row.get('stockQuantity') else None,
                'supplier': row.get('supplier'),
                'purchasePrice': float(row['purchasePrice']) if row.get('purchasePrice') else None,
                'purchasetaxName': row.get('purchasetaxName'),
                'reorderLevel': int(row['reorderLevel']) if row.get('reorderLevel') else None,
                'itemType': row.get('itemType'),
                'hsnCode': row.get('hsnCode'),
                'shelfLife': row.get('shelfLife'),
                'vendorTag': row.get('vendorTag').split(',') if row.get('vendorTag') else [],
